...
import { createWebp } from "./gulptasks/createWebp.js";
import { deploy } from "./gulptasks/deploy.js";
import { cleanCss } from "./gulptasks/cleanCss.js";
...
const images = () => {
  return (
    gulp
      .src("source/img/**/*.{jpg,jpeg,png,gif,webp}")
      // .pipe(newer('dist/img'))
      .pipe(app.if(app.isBuild, webp({ quality: 75 })))
      .pipe(app.if(app.isBuild, gulp.dest("dist/img")))
      .pipe(
        app.if(app.isBuild, gulp.src("source/img/**/*.{jpg,jpeg,png,gif,webp}"))
      )
...
const js = () => {
  return gulp
    .src("source/js/main.js", { sourcemaps: app.isDev })
    .pipe(
      webpack({
